...
public class Form extends JDialog implements ActionListener, ListSelectionListener
{
  ...
  AttEventDef att_event_def;
  private JPanel parPanel = new JPanel(null);
  private JPanel cfPanel = new JPanel(null);
  ...
  public Form(IISFrameMain parent, String title, boolean modal)
  {
  ...
  public Form(IISFrameMain parent, String title, boolean modal,Connection con,int m , PTree tr, String str )
  {
    super((Frame) parent, title, modal);
    try
    { connection=con; 
      ...
      try
      {
        if(!tree.tree.getSelectionPath().getLastPathComponent().equals("Referenced"))
    ...
    if(m>=0){
      ...
      rs1=query.select("select * from IISC_Form_TYPE where PR_id="+ tree.ID +" and TF_id="+id);
      rs1.next();
  ...
  public Form(IISFrameMain parent, String title, boolean modal,Connection con,int m , String _Mnem, PTree tr, int as_id, String app_sys)
  {
    super((Frame) parent, title, modal);
    try
    { connection=con; 
  ...
  {      JDBCQuery query=new JDBCQuery(connection);
    ...
    this.setSize(new Dimension(763, 359));
    this.getContentPane().setLayout(null);
    this.setTitle("Owned Form Types");
...
tableatt.setAutoscrolls(true);
  ...
  tableatt.setBorder(BorderFactory.createLineBorder(Color.black, 1));
   tableatt.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
    ...
    par = new Parameter(jTabbedPane1.getWidth(), jTabbedPane1.getHeight(),(IISFrameMain)getOwner(), this, connection, this.tree.ID);
    
    att_event_def = new AttEventDef(jTabbedPane1.getWidth() - 15, jTabbedPane1.getHeight(), -1, -1, this.id,  this.tree.ID, connection, (IISFrameMain)getOwner(), "3");
     
    cf = new CalledForms(jTabbedPane1.getWidth(), jTabbedPane1.getHeight(),(IISFrameMain)getOwner(), this, connection,this.tree.ID);
    
    parPanel.add(par);
    jTabbedPane1.addTab("Parameters", parPanel);
    
    cfPanel.add(cf);
    jTabbedPane1.addTab("Called Form Types", cfPanel);
...
{ rs=query.select("select * from IISC_COMPONENT_TYPE_OBJECT_TYPE  where  PR_id="+ tree.ID +" and TOB_mnem='"+ table.getValueAt(table.getSelectedRow(),0) +"' and TF_id=" + id);
 if(rs.next())
 qtmatt.setQueryA("select * from IISC_ATT_TOB where  PR_id="+ tree.ID +" and  TF_id="+ id +" and TOB_id="+rs.getInt("TOB_id") );
 else
...
try
{
 ...
 rs1=query.select("select * from IISC_Form_TYPE,IISC_TF_APPSYS  where IISC_TF_APPSYS.AS_id="+ as +" and IISC_Form_TYPE.TF_id=IISC_TF_APPSYS.TF_id and  PR_id="+ tree.ID +"  order by  TF_mnem asc");
 if(rs1.next())
{s=rs1.getString(3);
}
query.Close();
...
rs1=query.select("select TF_mnem from IISC_Form_TYPE,IISC_TF_APPSYS  where IISC_TF_APPSYS.AS_id="+ as +" and IISC_Form_TYPE.TF_id=IISC_TF_APPSYS.TF_id and  IISC_Form_TYPE.PR_id="+ tree.ID +"  and TF_mnem<'" + s + "' order by TF_mnem desc" );
if(rs1.next())
{
...
try
{
 ...
 rs1=query.select("select TF_mnem from IISC_Form_TYPE,IISC_TF_APPSYS  where IISC_TF_APPSYS.AS_id="+ as +" and IISC_Form_TYPE.TF_id=IISC_TF_APPSYS.TF_id and IISC_Form_TYPE.PR_id="+ tree.ID +" and TF_mnem>'" + s + "' order by TF_mnem asc" );
if(rs1.next())
{
s=rs1.getString(1);
}
...
{ if(btnSave.isEnabled())
 ...
 rs1=query.select("select * from IISC_Form_TYPE,IISC_TF_APPSYS  where IISC_TF_APPSYS.AS_id="+ as +" and IISC_Form_TYPE.TF_id=IISC_TF_APPSYS.TF_id and IISC_Form_TYPE.PR_id="+ tree.ID +" order by  TF_mnem desc");
if(rs1.next())
{s=rs1.getString(3);
}
query.Close();
...
int i;
try
{rs=query.select("select * from IISC_COMPONENT_TYPE_OBJECT_TYPE where TF_id="+ id +" and TOB_superord is null and PR_id="+ tree.ID +" ");
if(rs.next())
i=rs.getInt(1);
else
i=-1;
...
private Object[] tree_objects(String s)
{
    ...
    try
    {
        rs1=query.select("select count(*) from IISC_COMPONENT_TYPE_OBJECT_TYPE where TF_id="+ id +" and TOB_superord=" + s + " and PR_id="+ tree.ID +" ");
        ...
        if(rs1.next())
        {
            ...
            rs1=query.select("select * from IISC_COMPONENT_TYPE_OBJECT_TYPE where TF_id="+ id +" and TOB_id=" + s + " and PR_id="+ tree.ID +" ");
            rs1.next();
            obj[0]=rs1.getString("Tob_mnem");
            query.Close();
            rs1=query.select("select * from IISC_COMPONENT_TYPE_OBJECT_TYPE where TF_id="+ id +" and TOB_superord=" + s + " and PR_id="+ tree.ID +" order by  Tob_mnem asc");
            int i = 1;
            while(rs1.next())
            {
              int ik=rs1.getInt("Tob_id");
// rs=query.select("select * from IISC_COMPONENT_TYPE_OBJECT_TYPE where TF_id="+ id +" and TOB_superord=" +ik + " and PR_id="+ tree.ID +" order by  Tob_mnem asc");
//if(rs.next())
//{
...
if(Mnem.trim().equals(""))
{
 rs1=query.select("select * from IISC_Form_TYPE  where PR_id="+ tree.ID +" and TF_mnem='"+ txtName.getText().trim() +"'");
if(rs1.next())
...
{
rs1=query.select("select * from IISC_Form_TYPE  where PR_id="+ tree.ID +" and TF_mnem='"+ Mnem +"'");
...
int i=query.update("insert into IISC_Form_TYPE(Tf_id,PR_ID,Tf_mnem,Tf_title,Tf_crdate,Tf_freq,Tf_freq_unit,Tf_moddate,Tf_rest,Tf_rest_unit,Tf_use,Tf_comment) values ("+ id +","+ tree.ID +",'" + txtName.getText().trim() + "','" + txtTitle.getText().trim() + "','" + ODBCList.now() + "','" + txtFreq.getText() + "','" + cmbUnitF.getSelectedItem().toString() + "','" + ODBCList.now() + "','" + txtResp.getText() + "','" + cmbUnitR.getSelectedItem().toString() + "'," + use + ",'" + txtComment.getText() + "')");
i=query.update("insert into IISC_TF_APPSYS(Tf_id,AS_id,PR_id) values ("+ id +","+ as +","+ tree.ID +")");
...
public void setForm (String m)
 ...
 { 
    ...
    ResultSet rs;
    ResultSet rs1;
      rs=query.select("select count(*) from IISC_FORM_TYPE,IISC_TF_APPSYS  where IISC_FORM_TYPE.TF_id=IISC_TF_APPSYS.TF_id and AS_ID="+ as +" and IISC_FORM_TYPE.PR_ID="+ tree.ID);
    rs.next();
    int j=rs.getInt(1);
    query.Close();
    String[] sa=query.selectArray("select * from IISC_FORM_TYPE,IISC_TF_APPSYS where IISC_FORM_TYPE.TF_id=IISC_TF_APPSYS.TF_id and  AS_ID="+ as +" and IISC_FORM_TYPE.PR_ID="+ tree.ID ,j,3);
    sa[0]="";
    query.Close();
...
rs1=query.select("select * from IISC_Form_TYPE,IISC_TF_APPSYS  where IISC_TF_APPSYS.AS_id="+ as +" and IISC_Form_TYPE.TF_id=IISC_TF_APPSYS.TF_id and  IISC_FORM_TYPE.PR_id="+ tree.ID +" and TF_mnem>'" + m.trim() + "'" );
if(rs1.next())
...
rs=query.select("select * from IISC_COMPONENT_TYPE_OBJECT_TYPE  where  PR_id="+ tree.ID +" and TOB_mnem='"+ table.getValueAt(table.getSelectedRow(),0) +"' and Tf_id="+ id );
 if(rs.next())
 {
  f=rs.getInt(1);
   query.Close();
  int key=1;
  rs=query.select("select max(Tob_rbrk)+1 from IISC_ATT_KTO  where   PR_id="+ tree.ID +" and TOB_id="+ f +" and Tf_id="+ id );
...
{int fi=-1;
rs1=query.select("select * from IISC_ATT_TOB,IISC_ATTRIBUTE  where  IISC_ATT_TOB.PR_id="+ tree.ID +" and IISC_ATT_TOB.Att_id=IISC_ATTRIBUTE.Att_id and  Att_mnem='"+ tableatt.getValueAt(k,0) +"'  and Tf_id="+ id );
if(rs1.next())fi=rs1.getInt(1);
query.Close();
query.update("insert into IISC_ATT_KTO(Att_id,Tf_id,Tob_id,PR_id,Tob_rbrk,Att_rbrk) values("+ fi +","+ id +","+ f +","+ tree.ID +","+ key +")");
}
}
query.update("insert into IISC_KEY_TOB(Tf_id,Tob_id,Tob_rbrk,PR_id,Tob_local) values("+ id +","+ f +","+ key +","+ tree.ID +")");
 qtmatt.setQueryA("select * from IISC_ATT_TOB where PR_id="+ tree.ID +" and TOB_id="+f + " and Tf_id="+ id  );
 }
 else
 query.Close();
}
...
{if(tableatt.isRowSelected(k))
{int fi=-1;
rs1=query.select("select * from IISC_ATT_TOB,IISC_ATTRIBUTE  where  IISC_ATT_TOB.PR_id="+ tree.ID +" and IISC_ATT_TOB.Att_id=IISC_ATTRIBUTE.Att_id and  Att_mnem='"+ tableatt.getValueAt(k,0) +"'");
if(rs1.next())fi=rs1.getInt(1);
query.Close();
rs=query.select("select * from IISC_ATT_KTO where Att_id="+ fi +" and TF_id="+ id +" and TOB_id="+ f +" and PR_id="+ tree.ID +"");
while(rs.next())
{int key=rs.getInt("TOB_RBRK");
query.update("delete from IISC_ATT_KTO where Att_id="+ fi +" and TF_id="+ id +" and TOB_id="+ f +" and PR_id="+ tree.ID +" and TOB_rbrk="+ key +"");
rs1=query1.select("select * from IISC_ATT_KTO   where  TF_id="+ id +"  and PR_id="+ tree.ID +" and TOB_rbrk="+ key +"");
if(!rs1.next())
{query2.update("delete from IISC_KEY_TOB   where  TF_id="+ id +"  and PR_id="+ tree.ID +" and TOB_rbrk="+ key +"");
query2.update("update IISC_ATT_KTO set  TOB_rbrk=TOB_rbrk-1 where  TF_id="+ id +" and TOB_id="+ f +" and PR_id="+ tree.ID +" and TOB_rbrk>"+ key +"");
query2.update("update IISC_KEY_TOB set  TOB_rbrk=TOB_rbrk-1 where  TF_id="+ id +"   and PR_id="+ tree.ID +" and TOB_rbrk>"+ key +"");
}
query1.Close();
}
query.Close();
...
catch(SQLException ex)
{
 JOptionPane.showMessageDialog(null, ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
  ...
  JDBCQuery query=new JDBCQuery(connection);
    ResultSet rs;
 try{
rs=query.select("select * from IISC_COMPONENT_TYPE_OBJECT_TYPE where TF_id="+ id +" and TOB_mnem='"+ table.getValueAt(table.getSelectedRow(),0) +"' and PR_Id="+ tree.ID +"");
  if(rs.next())
 {
...
catch(SQLException ex)
{
 ...
 hlp.setVisible(true);
  ...
  if(cmbForm.getSelectedItem()!=null)
      s=cmbForm.getSelectedItem().toString();
  setForm(s); 
  Mnem=s;
...
query.Close();
rs=query.select("select  min(W_order) from IISC_ATT_TOB  where W_order>"+ k1 +" and IISC_ATT_TOB.TOB_id="+ tb +" and  IISC_ATT_TOB.PR_Id="+ tree.ID + " and IISC_ATT_TOB.Tf_id="+ id);
 if(rs.next())
 {int k= rs.getInt(1); 
 query.Close();
 if(k> k1)
{
  query.update("update IISC_ATT_TOB set W_order="+ k1 +" where W_order="+ k +" and IISC_ATT_TOB.TOB_id="+ tb +" and  IISC_ATT_TOB.PR_Id="+ tree.ID + " and IISC_ATT_TOB.Tf_id="+ id);
query.update("update IISC_ATT_TOB set W_order="+ (k1+1)  +" where  IISC_ATT_TOB.TOB_id="+ tb +" and  IISC_ATT_TOB.Att_Id="+ at + " and  IISC_ATT_TOB.PR_Id="+ tree.ID + " and IISC_ATT_TOB.Tf_id="+ id);
 qtmatt.setQueryA("select * from IISC_ATT_TOB where PR_id="+ tree.ID +" and TOB_id="+tb + " and Tf_id="+ id  );
}
} 
else
   query.Close();
...
rs=query.select("select IISC_ATT_TOB.TOB_id,IISC_ATT_TOB.Att_id,W_order,ATT_mnem from IISC_ATT_TOB,IISC_ATTRIBUTE,IISC_COMPONENT_TYPE_OBJECT_TYPE where IISC_ATTRIBUTE.Att_id=IISC_ATT_TOB.Att_id and IISC_COMPONENT_TYPE_OBJECT_TYPE.TOB_id=IISC_ATT_TOB.TOB_id and TOB_mnem='"+ table.getValueAt(table.getSelectedRow(),0) +"' and  IISC_ATTRIBUTE.PR_Id="+ tree.ID +" and IISC_ATT_TOB.Tf_id="+ id +" and Att_mnem='"+ tableatt.getValueAt(tableatt.getSelectedRow(),1) +"'");
 ...
 int at=rs.getInt(2);
  ...
  String att=rs.getString(4);
query.Close();
rs=query.select("select max(W_order) from IISC_ATT_TOB  where W_order<"+ k1 +" and IISC_ATT_TOB.TOB_id="+ tb +" and  IISC_ATT_TOB.PR_Id="+ tree.ID + " and IISC_ATT_TOB.Tf_id="+ id);
 if(rs.next())
 {int k= rs.getInt(1);
 query.Close();
query.update("update IISC_ATT_TOB set W_order="+(k+1)+" where W_order="+ k +" and IISC_ATT_TOB.TOB_id="+ tb +" and  IISC_ATT_TOB.PR_Id="+ tree.ID + " and IISC_ATT_TOB.Tf_id="+ id);
query.update("update IISC_ATT_TOB set W_order="+ k  +" where  IISC_ATT_TOB.TOB_id="+ tb +" and  IISC_ATT_TOB.Att_Id="+ at + " and  IISC_ATT_TOB.PR_Id="+ tree.ID + " and IISC_ATT_TOB.Tf_id="+ id);
 qtmatt.setQueryA("select * from IISC_ATT_TOB where PR_id="+ tree.ID +" and TOB_id="+tb + " and Tf_id="+ id  );
 }
 else
   query.Close();
...
catch(SQLException ex)
{
 JOptionPane.showMessageDialog(null, ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
  ...
  JDBCQuery query=new JDBCQuery(connection);
    ResultSet rs;
 try{
rs=query.select("select * from IISC_COMPONENT_TYPE_OBJECT_TYPE  where  PR_id="+ tree.ID +" and TF_id="+ id +" and TOB_mnem='"+ table.getValueAt(table.getSelectedRow(),0) +"'");
 if(rs.next())
 {
